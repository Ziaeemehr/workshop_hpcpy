...
\begin{frame}{Why Python is Still OK?}
\begin{block}{}
Most heavy computation happens either in compiled libraries, Numba, JAX, or on GPUs, so Python mainly \textbf{orchestrates the workflow}.
\end{block}
\end{frame}
...
\begin{frame}{JAX RNG: Stateless Random Generation}
\begin{itemize}
  \item Functional approach to randomness
  \item PRNG keys for reproducibility
  \item Safe for jit and vmap
\end{itemize}
\end{frame}
...
\begin{frame}{Advanced JAX Concepts}
\begin{itemize}
  \item \textbf{PyTree}: Nested data structures
  \item \textbf{LAX}: Low-level operations
  \item \textbf{Scan}: Efficient loops
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Advanced concepts (PyTree, LAX, Scan)
\end{itemize}
\end{block}
\end{frame}

%================================================
\section{Native Extensions (Optional)}

\begin{frame}{Native Extensions: Python Meets C/C++}
\begin{itemize}
  \item Interface generators and compilers
  \item Wrap existing C/C++ code
  \item Use legacy HPC libraries
  \item Tools: SWIG, PyBind11, Cython
\end{itemize}
\end{frame}

\begin{frame}{When Native Extensions Make Sense}
\begin{itemize}
  \item Existing C/C++ codebase
  \item Performance-critical kernels
  \item Long-term maintenance
  \item Memory-intensive computations
\end{itemize}
\end{frame}
